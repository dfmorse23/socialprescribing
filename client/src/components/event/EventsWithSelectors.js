...
export default function EventsWithSelectors(props) {
  ...
  const handleSearch = async (searchValue) => {
    ...
    try {
      ...
      setEventSearchError(
        `Unable to connect to our event servers. Check your internet connection.`
      );
    }
  ...
  const getZipFromCoordinates = async (latlon) => {
    ...
    try {
      const response = await fetch(
        `https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lon}&key=${process.env.REACT_APP_MAPS_JS_API}`,
        {
          method: "GET",
        }
      );
  ...
  const handleFilterSelection = async (e, newSelections) => {
    ...
    const difference = newSelections.filter(
      (x) => !filterSelections.includes(x)
    );
    ...
    // }
      ...
      const newEvents = allEvents.filter((event) =>
        newSelections.includes(event.tag)
      );
      setEvents(newEvents || []);
    }
      ...
      <div style={{ marginBottom: "50px" }}>
        ...
        {userHasSearched || displayingFavorites ? (
            ...
            <Grid container spacing={4}>
              {events &&
                events.map((event, index) => (
                  <EventCard
                    key={`${index}-${event.title}-${displayingFavorites}`}
                    sig={index + imageSeed}
                    event={event}
          ...
          >
            ...
            <h1 style={{ margin: "50px 20px", color: "#aaaaaa" }}>
              {" "}
              Find prescriptions near you.
            </h1>
          </Grid>
        )}
      </div>
